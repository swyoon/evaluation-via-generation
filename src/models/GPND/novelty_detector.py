...
def extract_statistics(cfg, train_set, inliner_classes, E, G):
    ...
    data_loader = make_dataloader(
        train_set, cfg.TEST.BATCH_SIZE, torch.cuda.current_device()
    )
...
def extract_statistics_V2(train_dl, model, device="cpu"):
    latent_size = model.z_shape[0]
    zlist = []
    rlist = []
    ...
    gennorm_param = np.zeros([3, latent_size])
...
def main(folding_id, inliner_classes, ic, total_classes, mul, folds=5):
    ...
    counts, bin_edges, gennorm_param = extract_statistics(
        cfg, train_set, inliner_classes, E, G
    )

    def run_novely_prediction_on_dataset(dataset, percentage, concervative=False):
        dataset.shuffle()
        dataset = create_set_with_outlier_percentage(
            dataset, inliner_classes, percentage, concervative
        )
        ...
        data_loader = make_dataloader(
            dataset, cfg.TEST.BATCH_SIZE, torch.cuda.current_device()
        )
        ...
        for label, x in data_loader:
            ...
            for i in range(x.shape[0]):
                ...
                p = scipy.stats.gennorm.pdf(
                    z[i], gennorm_param[0, :], gennorm_param[1, :], gennorm_param[2, :]
                )
    ...
    def compute_threshold(valid_set, percentage):
        y_scores, y_true = run_novely_prediction_on_dataset(
            valid_set, percentage, concervative=True
        )
    ...
    def test(test_set, percentage, threshold):
        y_scores, y_true = run_novely_prediction_on_dataset(
            test_set, percentage, concervative=True
        )
        return evaluate(
            logger, percentage, inliner_classes, y_scores, threshold, y_true
        )
