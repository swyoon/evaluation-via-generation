...
class GeneratorMNIST(nn.Module):
    def __init__(self, z_size, d=128, channels=1):
        ...
        self.deconv1_1 = nn.ConvTranspose2d(z_size, d * 2, 7, 1, 0)
        self.deconv1_1_bn = nn.BatchNorm2d(d * 2)
        self.deconv2 = nn.ConvTranspose2d(d * 2, d * 2, 4, 2, 1)
        self.deconv2_bn = nn.BatchNorm2d(d * 2)
        self.deconv3 = nn.ConvTranspose2d(d * 2, channels, 4, 2, 1)
        self.z_size = z_size
    ...
    def forward(self, x):
        x = F.relu(self.deconv1_1_bn(self.deconv1_1(x)))
        x = F.relu(self.deconv2_bn(self.deconv2(x)))
        x = torch.tanh(self.deconv3(x)) * 0.5 + 0.5
        return x


class Discriminator(nn.Module):
    def __init__(self, d=128, channels=1, mnist=False):
...
class Encoder(nn.Module):
    def __init__(self, z_size, d=128, channels=1, mnist=False):
        ...
        self.conv3_bn = nn.BatchNorm2d(d * 4)
        if mnist:
            self.conv4 = nn.Conv2d(d * 4, z_size, 3, 1, 0)
        else:
...
class ZDiscriminator(nn.Module):
    def __init__(self, z_size, batchSize=None, d=128):
        super(ZDiscriminator, self).__init__()
        # self.linear1 = nn.Linear(z_size, d)
        # self.linear2 = nn.Linear(d, d)
        # self.linear3 = nn.Linear(d, 1)
        self.linear1 = nn.Conv2d(z_size, d, 1)
        self.linear2 = nn.Conv2d(d, d, 1)
        self.linear3 = nn.Conv2d(d, 1, 1)
...
class ZDiscriminator_mergebatch(nn.Module):
    def __init__(self, z_size, batchSize, d=128):
        ...
        # self.linear3 = nn.Linear(d, 1)
        self.linear1 = nn.Conv2d(z_size, d, 1, 0)
        self.linear2 = nn.Conv2d(d * batchSize, d, 1, 0)
        self.linear3 = nn.Conv2d(d, 1, 1, 0)
    ...
    def forward(self, x):
        x = F.leaky_relu((self.linear1(x)), 0.2).view(
            1, -1
        )  # after the second layer all samples are concatenated
        x = F.leaky_relu((self.linear2(x)), 0.2)
        x = torch.sigmoid(self.linear3(x))
        return x
