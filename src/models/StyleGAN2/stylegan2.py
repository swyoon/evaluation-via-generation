...
import models.StyleGAN2.utils.style_misc as misc
from models.StyleGAN2.utils.style_ops import bias_act, conv2d_resample, fma, upfirdn2d
...
class Discriminator(torch.nn.Module):
    def __init__(
        ...
        if self.d_cond_mtd == "MH":
            self.linear1 = FullyConnectedLayer(
                channels_dict[4], 1 + self.num_classes, bias=True
            )
        elif self.d_cond_mtd == "MD":
            self.linear1 = FullyConnectedLayer(
                channels_dict[4], self.num_classes, bias=True
            )
        elif self.d_cond_mtd == "SPD":
            self.linear1 = FullyConnectedLayer(
                channels_dict[4], 1 if self.cmap_dim == 0 else self.cmap_dim, bias=True
            )
        else:
        ...
        if self.d_cond_mtd == "AC":
            self.linear2 = FullyConnectedLayer(
                channels_dict[4], num_classes, bias=False
            )
        elif self.d_cond_mtd == "PD":
            self.linear2 = FullyConnectedLayer(
                channels_dict[4], self.cmap_dim, bias=True
            )
        elif self.d_cond_mtd == "SPD":
            self.mapping = MappingNetwork(
                z_dim=0,
        ...
        if self.aux_cls_type == "TAC":
            if self.d_cond_mtd == "AC":
                self.linear_mi = FullyConnectedLayer(
                    channels_dict[4], num_classes, bias=False
                )
            elif self.d_cond_mtd in ["2C", "D2DCE"]:
                self.linear_mi = FullyConnectedLayer(
                    channels_dict[4], d_embed_dim, bias=True
                )
                self.embedding_mi = MappingNetwork(
                    z_dim=0,
                    c_dim=c_dim,
                    w_dim=d_embed_dim,
                    num_ws=None,
    ...
    def forward(self, img, label, eval=False, adc_fake=False, **block_kwargs):
        ...
        oh_label = F.one_hot(
            label,
            self.num_classes * 2 if self.aux_cls_type == "ADC" else self.num_classes,
        )
        ...
        if self.d_cond_mtd == "AC":
            ...
            adv_output = adv_output + torch.sum(
                torch.mul(self.embedding(None, oh_label), h), 1
            )
        elif self.d_cond_mtd == "SPD":
            embed = self.linear1(h)
            cmap = self.mapping(None, oh_label)
            adv_output = (embed * cmap).sum(dim=1, keepdim=True) * (
