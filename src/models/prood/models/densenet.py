...
class DenseNet3(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.normalizer is not None:
            x = x.clone()
            x[:, 0, :, :] = (
                x[:, 0, :, :] - self.normalizer.mean[0]
            ) / self.normalizer.std[0]
            x[:, 1, :, :] = (
                x[:, 1, :, :] - self.normalizer.mean[1]
            ) / self.normalizer.std[1]
            x[:, 2, :, :] = (
                x[:, 2, :, :] - self.normalizer.mean[2]
            ) / self.normalizer.std[2]
...
class Wrapper(nn.Module):
    ...
    def forward(self, x):
        return 1.0 - torch.softmax(self.model(x), dim=1)[:, -1]


def get_densenet(dataset, num_classes):
