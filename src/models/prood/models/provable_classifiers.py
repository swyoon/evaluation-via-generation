...
from . import modules_ibp as modules_ibp
...
class CNN_IBP(nn.Module):
    def __init__(
        ...
        if size == "L":
            self.C1 = modules_ibp.Conv2dI(
                self.color_channels, 64, 3, padding=1, stride=1
            )
            ...
            self.C1 = modules_ibp.Conv2dI(
                self.color_channels, 128, 3, padding=1, stride=1
            )
            self.A1 = modules_ibp.ReLUI()
            ...
            self.width = 2
            self.C1 = modules_ibp.Conv2dI(
                self.color_channels, 128 * self.width, 3, padding=1, stride=1
            )
            self.A1 = modules_ibp.ReLUI()
            self.C2 = modules_ibp.Conv2dI(
                128 * self.width, 128 * self.width, 3, padding=1, stride=1
            )
            self.A2 = modules_ibp.ReLUI()
            self.C3 = modules_ibp.Conv2dI(
                128 * self.width, 256 * self.width, 3, padding=1, stride=2
            )
            self.A3 = modules_ibp.ReLUI()
            self.C4 = modules_ibp.Conv2dI(
                256 * self.width, 256 * self.width, 3, padding=1, stride=1
            )
            self.A4 = modules_ibp.ReLUI()
            self.C5 = modules_ibp.Conv2dI(
                256 * self.width, 256 * self.width, 3, padding=1, stride=1
            )
            self.A5 = modules_ibp.ReLUI()
            self.F = modules_ibp.FlattenI()
            self.L6 = modules_ibp.LinearI(
                256 * self.width * (self.hw // 2) ** 2, 512 * self.width
            )
            self.A6 = modules_ibp.ReLUI()
            ...
            self.width = 1
            self.C1 = modules_ibp.Conv2dI(
                self.color_channels, 64 * self.width, 3, padding=1, stride=1
            )
            self.A1 = modules_ibp.ReLUI()
            self.C2 = modules_ibp.Conv2dI(
                64 * self.width, 128 * self.width, 3, padding=1, stride=2
            )
            self.A2 = modules_ibp.ReLUI()
            self.C3 = modules_ibp.Conv2dI(
                128 * self.width, 128 * self.width, 3, padding=1, stride=1
            ...
            self.width = 1
            self.C1 = modules_ibp.Conv2dI(
                self.color_channels, 512 * self.width, 3, padding=1, stride=1
            )
            self.A1 = modules_ibp.ReLUI()
            self.C2 = modules_ibp.Conv2dI(
                512 * self.width, 1024 * self.width, 3, padding=1, stride=2
            )
            self.A2 = modules_ibp.ReLUI()
            self.C3 = modules_ibp.Conv2dI(
                1024 * self.width, 512 * self.width, 3, padding=1, stride=1
            )
            self.A3 = modules_ibp.ReLUI()
            self.F = modules_ibp.FlattenI()
            ...
            self.C1 = modules_ibp.Conv2dI(
                self.color_channels, 512, 3, padding=1, stride=1
            )
            self.A1 = modules_ibp.ReLUI()
            ...
            self.C1 = modules_ibp.Conv2dI(
                self.color_channels, 256, 3, padding=1, stride=1
            )
            self.A1 = modules_ibp.ReLUI()
    ...
    def ibp_elision_forward(self, l, u, num_classes):
        ...
        W = layer.weight
        Wd = W.unsqueeze(dim=1).expand((num_classes, num_classes, -1)) - W.unsqueeze(
            dim=0
        ).expand((num_classes, num_classes, -1))
        ud = torch.einsum("abc,nc->nab", Wd.clamp(min=0), u) + torch.einsum(
            "abc,nc->nab", Wd.clamp(max=0), l
        )
        if layer.bias is not None:
            bd = layer.bias.unsqueeze(dim=1).expand(
                (num_classes, num_classes)
            ) - layer.bias.unsqueeze(dim=0).expand((num_classes, num_classes))
            ud += bd.unsqueeze(0)
        ...
        if layer.bias is not None:
            ...
            u_ = (
                layer.weight.clamp(min=0) @ u.t() + layer.weight.clamp(max=0) @ l.t()
            ).t()
        l, u = l_, u_
        return l, u, ud
