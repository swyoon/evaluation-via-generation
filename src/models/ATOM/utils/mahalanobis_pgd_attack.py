...
class MahalanobisLinfPGDAttack:
        ...
        for layer_index in range(self.num_output):
            ...
            for i in range(self.num_classes):
                ...
                term_gau = (
                    -0.5
                    * torch.mm(
                        torch.mm(zero_f, self.precision[layer_index]), zero_f.t()
                    ).diag()
                )
                if i == 0:
            ...
            gaussian_score = (
                -0.5
                * torch.mm(
                    torch.mm(zero_f, Variable(self.precision[layer_index])), zero_f.t()
                ).diag()
            )
    ...
    def attack_single_run(self, x):
        ...
        if self.rand_init:
            ...
            delta.data = (
                torch.clamp(
                    x.data + delta.data / 255.0, min=self.clip_min, max=self.clip_max
                )
                - x.data
            ) * 255.0
    ...
    def perturb(self, x):
        ...
        adv_x = torch.clamp(
            x
            + torch.clamp(torch.round(worst_perb), min=-self.eps, max=self.eps) / 255.0,
            min=self.clip_min,
            max=self.clip_max,
        )
