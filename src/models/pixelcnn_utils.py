...
def discretized_mix_logistic_loss(x, l, predict=False):
    """log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval
    if predict is False: return summed negative log likelhood
    if predict is True: return negative log likelihood for each sample"""
    # Pytorch ordering
    x = x.permute(0, 2, 3, 1)
    l = l.permute(0, 2, 3, 1)
    ...
    if predict:
        return -log_sum_exp(log_probs).sum(axis=2).sum(axis=1)
    else:
        return -torch.sum(log_sum_exp(log_probs))


def discretized_mix_logistic_loss_1d(x, l, predict=False):
    """log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval
    if predict is False: return summed negative log likelhood
    if predict is True: return negative log likelihood for each sample"""
    # Pytorch ordering
