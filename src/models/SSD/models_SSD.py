...
class SSLResNet(nn.Module):
    ...
    def initialize_ftrain(self, ftrain):
        ...
        self.inv_cov = np.linalg.pinv(cov(ftrain))

    def predict(self, x):
        food = self.encoder(x).data.cpu().numpy()
        food /= np.linalg.norm(food, axis=-1, keepdims=True) + 1e-10
        food = (food - self.m) / (self.s + 1e-10)

        # assume one cluster used
        dood = np.sum(
            (food - self.m) * (self.inv_cov.dot((food - self.m).T)).T,
            axis=-1,
        )
        return torch.tensor(dood).to(x)
