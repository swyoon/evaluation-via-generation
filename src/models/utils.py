...
class Fire_UP(nn.Module):
    def __init__(self, inplanes, squeeze_planes, expand1x1_planes, expand3x3_planes):
        ...
        self.upsample = nn.ConvTranspose2d(
            inplanes, inplanes, kernel_size=4, stride=2, padding=1
        )
        self.squeeze = nn.ConvTranspose2d(inplanes, squeeze_planes, kernel_size=1)
        self.squeeze_activation = nn.ReLU(inplace=True)
        self.expand1x1 = nn.ConvTranspose2d(
            squeeze_planes, expand1x1_planes, kernel_size=1
        )
        self.expand1x1_activation = nn.ReLU(inplace=True)
        self.expand3x3 = nn.ConvTranspose2d(
            squeeze_planes, expand3x3_planes, kernel_size=3, padding=1
        )
        self.expand3x3_activation = nn.ReLU(inplace=True)
...
class BasicBlock_UP(nn.Module):
    ...
    def __init__(self, inplanes, planes, upsample=2):
        ...
        self.conv1 = nn.ConvTranspose2d(
            inplanes, planes, kernel_size=kernel, stride=stride, padding=pad, bias=False
        )
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        ...
        self.bn2 = nn.BatchNorm2d(planes)
        self.upsample = nn.ConvTranspose2d(
            inplanes, planes, kernel_size=stride, stride=stride, padding=0, bias=False
        )
...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, is_dropout=False, p=0.5):
        ...
        if is_batchnorm:
            ...
            if is_dropout:
                self.conv2 = nn.Sequential(
                    nn.Conv2d(out_size, out_size, 3, 1, 1), nn.Dropout2d(p=p), nn.ReLU()
                )
            else:
                self.conv2 = nn.Sequential(
