...
class FlowStep(nn.Module):
    ...
    def normal_flow(self, input, logdet):
        ...
        # 3. coupling
        z1, z2 = utils.split_feature(z, "split")
...
class Glow_y0ast(nn.Module):
    def __init__(
        ...
        if learn_top:
            ...
            self.learn_top_fn = modules.Conv2dZeros(C * 2, C * 2)
    ...
    def prior(self, data, y_onehot=None):
        ...
        return utils.split_feature(h, "split")
    ...
    def normal_flow(self, x, y_onehot):
        ...
        x, logdet = utils.uniform_binning_correction(x)

        z, objective = self.flow(x, logdet=logdet, reverse=False)

        mean, logs = self.prior(x, y_onehot)
        objective += modules.gaussian_likelihood(mean, logs, z)
    ...
    def reverse_flow(self, z, y_onehot, temperature):
        with torch.no_grad():
            if z is None:
                ...
                z = modules.gaussian_sample(mean, logs, temperature)
            # sldj: sum of log determinant jacobian
            x, sldj = self.flow(z, temperature=temperature, reverse=True)
        return x, sldj

    def set_actnorm_init(self):
        for name, m in self.named_modules():
            if isinstance(m, modules.ActNorm2d):
                m.inited = True

    ## Copied from Glowv2
    # nll -> bpd
    def log_likelihood(self, x):
        z, bpd, y_logits = self(x, reverse=False)
        return -bpd
    ...
    def train_step(self, x, y=None, optimizer=None, clip_grad=None, scheduler=None):
        optimizer.zero_grad()
        loss = -self.log_likelihood(x).mean()
        loss.backward()
    ...
    def validation_step(self, x, y=None):
        with torch.no_grad():
            bpd = -self.log_likelihood(x)
            loss = bpd.mean()
        return {"loss": loss.item(), "predict": bpd.detach().cpu()}
