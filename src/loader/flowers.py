...
def get_flowers(
    ...
    transform = get_imageNet_augmentation(
        augm_type, out_size=size, config_dict=augm_config
    )
    path = paths_config.location_dict["Flowers"]
...
class Flowers(Dataset):
    def __init__(self, root, split, transform=None):
        self.root = root
        self.labels = loadmat(os.path.join(root, "imagelabels.mat"))["labels"][
            0
        ].astype(np.compat.long)
