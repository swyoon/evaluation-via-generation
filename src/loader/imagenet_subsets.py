...
# modified from https://github.com/AlexMeinke/Provable-OOD-Detection
import pickle
...
def get_restrictedImageNet(
    ...
    transform = get_imageNet_augmentation(
        type=augm_type, out_size=size, config_dict=augm_config
    )
    ...
    if train == True:
        dataset = RestrictedImageNet(
            path, split="train", transform=transform, balanced=balanced
        )
    else:
        dataset = RestrictedImageNet(
            path, split="val", transform=transform, balanced=balanced
        )
...
def get_restrictedImageNetValidationTestSplit(
    ...
    path = get_imagenet_path()
    dataset = RestrictedImageNet(
        path=path, split="val", transform=transform, balanced=False
    )
    loader = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, sampler=sampler, num_workers=num_workers
    )
...
class RestrictedImagenetBalancedSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for i in range(self.num_classes):
            ...
            if self.shuffle:
                permutation_idcs = torch.randperm(len(self.class_idcs[i]))[
                    : self.min_class_length
                ]
...
def get_ImageNet100(
    ...
    if train == True:
        dataset = ImageNetWIDSubset(
            path, split="train", wids=IMAGENET100_WIDS, transform=transform
        )
    else:
        dataset = ImageNetWIDSubset(
            path, split="val", wids=IMAGENET100_WIDS, transform=transform
        )
...
def get_ImageNetWithout(
    ...
    if train == True:
        dataset = ImageNetWIDSubset(
            path, split="train", wids=od_wids, transform=transform
        )
    else:
        dataset = ImageNetWIDSubset(
            path, split="val", wids=od_wids, transform=transform
        )
