...
def attack(m, method, x, y=None, device=None, use_label=False):
    ...
    attack1 = MonotonePGD(eps, iterations, stepsize, 10, momentum=0.9, 
                norm='inf', loss=loss, normalize_grad=False, early_stopping=0, restarts=0,
                init_noise_generator=noise, model=None, save_trajectory=False)
    ...
    #             norm='inf', normalize_grad=False, restarts=0,
    #             init_noise_generator=noise, save_trajectory=False, use_label=use_label)
    attack2 = MonotonePGD(eps, iterations, stepsize, 10, momentum=0.9, 
                norm='inf', loss=loss, normalize_grad=False, early_stopping=0, restarts=0,
                init_noise_generator=noise, model=None, save_trajectory=False)
    ...
    #             norm='inf', normalize_grad=False, restarts=0,
    #             init_noise_generator=noise, save_trajectory=False, use_label=use_label)
    attack3 = MonotonePGD(eps, iterations, stepsize, 10, momentum=0.9, 
                norm='inf', loss=loss, normalize_grad=False, early_stopping=0, restarts=0,
                init_noise_generator=noise, model=None, save_trajectory=False)
    ...
    attack = APGDAttack(None, n_iter=100*budget, n_iter_2=22*budget, n_iter_min=6*budget, size_decr=3,
                norm='Linf', n_restarts=restarts, eps=eps, show_loss=False, seed=0,
                loss=loss, show_acc=False, eot_iter=1, save_steps=False,
                save_dir='./results/', thr_decr=.75, check_impr=False,
                normalize_logits=False, device=device, apply_softmax=None, classes=10)
    ...
    for batch in dl:
        ...
        if no_grad:
            with torch.no_grad():
                if flatten:
                    x = x.view(len(x), -1)
                pred = attack(m, method, x.cuda(device), y=y.cuda(device), device=device, use_label=use_label).detach().cpu()
        else:
            if flatten:
                x = x.view(len(x), -1)
            pred = attack(m, method, x.cuda(device), y=y.cuda(device), device=device, use_label=use_label).detach().cpu()
